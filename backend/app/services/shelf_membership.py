...
The helper raises domain errors (:class:`~app.errors.NotFoundError` /
:class:`~app.errors.PermissionDeniedError`, S4) — the app-level handler maps them to 404/403 for
HTTP callers, and worker/CLI callers get meaningful framework-free exceptions. It does NOT
...
def add_work_to_shelf_checked(
    ...
    *,
    ...
    Returns the (created or existing) :class:`ShelfWork` link. Raises
    :class:`~app.errors.NotFoundError` if the shelf is missing and
    :class:`~app.errors.PermissionDeniedError` if the actor lacks modify access (mapped to
    404/403 by the app-level handler). ``settings`` is accepted for signature symmetry with
    ...
    if not access.can_modify_shelf(db, actor, shelf):
        raise PermissionDeniedError("You do not have permission to modify this shelf")
