...
export const temporalMapRenderer: VizRenderer = {
  ...
  buildOption(payload: VizPayload, theme: VizTheme): EChartsOptionLike {
    ...
    const series: Record<string, unknown>[] = seriesGroups.map((group) => {
      const groupNodes = colored ? nodes.filter((n) => n.color_group === group) : nodes;
      return {
        type: 'scatter',
        name: group ?? 'Papers',
    ...
    if (payload.edges && payload.edges.length > 0) {
      ...
      if (lines.length > 0) {
        series.push({
          ...
          data: lines,
          lineStyle: { color: theme.axisLine, opacity: 0.35, width: 1, curveness: 0 },
          silent: true,
          z: 0,
        });
      }
    ...
    return {
      ...
      legend: colored && groups.length > 0 ? { top: 0, textStyle: { color: theme.text } } : undefined,
      tooltip: {
        trigger: 'item',
      ...
      grid: { left: 56, right: 24, top: colored && groups.length > 0 ? 36 : 16, bottom: 48 },
      xAxis: {
        type: 'value',
