...
<script lang="ts">
  ...
  // Short summaries also accumulate one row per model (e.g. after switching summary model); the
  // extras beyond the one on show are the short history, mirroring the detailed history popup.
  $: shortHistory = summaries.filter((s) => !isDetailedType(s.summary_type) && s !== shortSummary);
  // Provenance: on a fallback the row still records the *requested* LLM in model_name, so show the
  // engine that actually produced the text (provider_used) instead of contradicting the "extractive
  // fallback" note above it.
  const provenanceName = (s: Summary) =>
    s.fallback ? (s.provider_used ?? 'extractive') : (s.model_name ?? s.provider_used ?? 'local');
  // Explain WHY a summary degraded to the extractive fallback (persisted now), instead of the old
  // blanket "(LLM unavailable)" — which was misleading when the model ran but returned nothing.
  const fallbackNote = (s: Summary) =>
    s.fallback_reason
      ? `Summarized with the extractive fallback: ${s.fallback_reason}`
      : 'Summarized with the extractive fallback — the AI model did not return a usable summary.';
  let summarisingDetail: SummaryDetail | null = null;

  // #22: make a historical summary version the currently-shown one, then reload so it moves to the top.
  async function promoteSummary(summaryId: string): Promise<void> {
    if (!work) return;
    await run(async () => {
      await client.promoteSummary(work.id, summaryId);
...
<div class="detail">
    ...
    <div class="summary-block">
      <div class="head">
        ...
        {#if shortHistory.length}
          <button type="button" class="linkish small" on:click={() => (shortHistoryOpen = !shortHistoryOpen)}
            title="View previously generated short summaries (other models)">History ({shortHistory.length})</button>
        {/if}
      </div>
      {#if shortHistoryOpen}
        <ul class="summary-history">
          {#each shortHistory as h}
            <li>
              <button type="button" class="linkish" on:click={() => (historyView = historyView === h ? null : h)}>
                {provenanceName(h)}{#if h.created_at} · {new Date(h.created_at).toLocaleDateString()}{/if}
              </button>
              {#if canModify}
                <button type="button" class="linkish small setcur" disabled={loading}
                  on:click={() => promoteSummary(h.id)}
                  title="Show this version as the current short summary">Set as current</button>
              {/if}
              {#if historyView === h}
                <div class="history-text">{#if mathMode === 'fancy'}<p class="summary-text">{@html renderSummaryMath(h.text)}</p>{:else}<p class="summary-text">{h.text}</p>{/if}</div>
              {/if}
            </li>
          {/each}
        </ul>
      ...
      {#if shortSummary}
        {#if shortSummary.fallback}
          <p class="degraded-hint" role="status">{fallbackNote(shortSummary)}</p>
        {/if}
        <p class="muted small">Generated by {provenanceName(shortSummary)}{#if shortSummary.created_at} · {new Date(shortSummary.created_at).toLocaleDateString()}{/if}</p>
        {#if mathMode === 'fancy'}
  ...
  <details open>
    <summary>Detailed summaries</summary>
    <div class="math-toggle">
      <button type="button" class="linkish small" class:active={mathMode === 'fancy'}
        on:click={() => (mathMode = mathMode === 'fancy' ? 'plain' : 'fancy')}
        title="Toggle LaTeX math rendering (switch to plain if equations look garbled)"
    ...
    <div class="summary-block">
      ...
      {#if historyOpen}
        <ul class="summary-history">
          {#each detailedHistory as h}
            <li>
              <button type="button" class="linkish" on:click={() => (historyView = historyView === h ? null : h)}>
                {h.summary_type.replace(/^.*_detailed_/, '')} · {provenanceName(h)}{#if h.created_at} · {new Date(h.created_at).toLocaleDateString()}{/if}
              </button>
              {#if canModify}
                <button type="button" class="linkish small setcur" disabled={loading}
                  on:click={() => promoteSummary(h.id)}
                  title="Show this version as the current detailed summary for its effort level">Set as current</button>
              {/if}
              {#if historyView === h}
      ...
      {#if detailedSummary}
        {#if detailedSummary.fallback}
          <p class="degraded-hint" role="status">{fallbackNote(detailedSummary)}</p>
        {/if}
        <p class="muted small">Generated by {provenanceName(detailedSummary)}{#if detailedSummary.created_at} · {new Date(detailedSummary.created_at).toLocaleDateString()}{/if}</p>
...
<style>
  ...
  .summary-history .setcur { margin-left: 0.6rem; }
  .summary-block {
