...
| **graph_color.py** | Shared `membership_groups(work_ids, color_by, actor)` — the ONE place resolving `shelf`/`rack`/`row`/`tag` color-groups per node for every graph surface (citation/reference/topic/viz), so the privacy rule lives once. Rack membership is **inferred** from a paper's shelves via `rack_shelves`; **row** membership one hop further (`shelf_works→shelves→rack_shelves→racks→row_racks→rows`, 2026-07-18) — a paper is never directly on a rack/row. Access-aware (2026-07-17 fix): an admin/owner `actor` sees all of *their* shelves/racks incl. **private** (previously a blanket non-private filter dropped an owner's own private rack → every paper collapsed to "unracked" while shelf coloring still worked); a non-privileged viewer sees non-private OR granted; `actor=None` stays conservative (non-private only) so a missing viewer never leaks a private name. Every graph builder threads its request `actor` through. |
| **reference_graph.py** | Per-paper reference "star": classifies each ref local / likely_local / external / citing, per-section weighted mention counts, per-local metrics (citation count, degree, topic Jaccard). Client-side shelf/rack/tag coloring reads `node.memberships` (resolved via `graph_color`, access-aware). |
...
| **recommendation.py** | AI "Recommend categorization" (Insights sub-tab, 2026-07-18). Per paper (features = title/abstract/keywords/topics) recommends TAGS (unassigned assignable tags) or CATEGORIES (rows/racks/shelves). Each kind ranked independently by an injectable `Ranker` — a JSON-output Ollama call (`format:"json"`, free-text-parse fallback) or an embedding-cosine fallback when no generative model is configured (flagged). Scoring is pure + unit-tested: base points `K−p+1` (or the model's 0–100 affinity when chosen + returned), then a 0.5 down-hierarchy boost (rack += 0.5×combine(picked parent rows); shelf += 0.5×combine(picked parent racks); combine = sum\|median\|max) → final shelf ranking. Candidates are access-filtered; an optional embedding prefilter shrinks prompts. `run_recommendation` drives a whole scope with progress/cancel; results are cached in `recommendation_runs` (`recommend_job` fills the row; `POST /recommend` create-or-cache, `GET /recommend/{id}` requester-gated). ⚠️ untrusted paper text enters the LLM prompt (prompt-injection surface; blast radius = a misleading, human-reviewed suggestion). |
| **scope_resolution.py** | Shared scope→works resolver (S1/S2, batch 2026-07-13) — the ONE place translating a scope into works, replacing per-feature copies (citation_graph, summarization, topic_modeling now delegate to it) whose merged-shadow/visibility handling had drifted apart. Covers all 8 scope kinds (`library`/`shelf`/`rack`/`row`/`search_result`/`selected_papers`/`saved_filter`/`import_batch`; `row` resolves work→shelf→rack→row); `scope_works_query` is query-returning (count/paginate without materializing); merged shadows always excluded; `visible_ids` is a **required** kwarg — omitting it is a `TypeError`, not a silent IDOR (topic_graph.py still has its own un-migrated resolver and the older optional-kwarg risk below). |
...
| **web_find.py** | Find-on-web aggregation + gated download (the security-heavy module). Per-source search (never raises); `resolve_final_url` re-checks SSRF + denylist + scheme on **every redirect hop**; `download_and_attach` re-classifies each hop, enforces content-type + `max_bytes` + `%PDF`. Policy modes restricted/careful/unrestricted; shadow-library denylist always wins. `WebCandidate.__post_init__` normalizes blank URL fields (`pdf_url`/`landing_url`/`resolved_url`) to `None` — upstream APIs send `""` for "no link" (Semantic Scholar's `openAccessPdf: {"url": ""}`), which otherwise reaches the client as a directly-downloadable candidate with an empty URL (the "missing download url" error). UX batch 3 / batch 4.1: when the direct fetch isn't a PDF, bounded fallbacks run (≤8 URLs, one landing-page read, no recursion, every URL re-gated): Elsevier Article Retrieval API for `10.1016/` DOIs (three gates: key set — admin write-only or `PARACORD_ELSEVIER_API_KEY` — ∧ `elsevier_api_enabled` master switch ∧ per-user `users.elsevier_api_allowed`, OFF by default), then `api_pdf_candidates` (S2 Graph API + DBLP hop + Unpaywall — metadata lookups, not downloads) and `pdf_link_finder` HTML discovery. **A landing host refused by the policy mode gate is NOT a dead end**: the direct fetch is skipped but discovery still runs, so a bot-walled `semanticscholar.org` page (built-in allow-listed, but serves an empty 202) can still surface its real PDF on `papers.nips.cc`; if every discovered URL is also refused, the refusal names the *PDF* host (the one worth allowing), and if discovery finds nothing the original landing-host refusal (with its policy hint) stands. A refused fallback host is skipped, never confirmed. See [08 SSRF](08_security.md#84-ssrf-protection). |
