...
| **graph_color.py** | Shared `membership_groups(work_ids, color_by, actor)` — the ONE place resolving `shelf`/`rack`/`tag` color-groups per node for every graph surface (citation/reference/topic/viz), so the privacy rule lives once. Rack membership is **inferred** from a paper's shelves via `rack_shelves` (a paper is never directly on a rack). Access-aware (2026-07-17 fix): an admin/owner `actor` sees all of *their* shelves/racks incl. **private** (previously a blanket non-private filter dropped an owner's own private rack → every paper collapsed to "unracked" while shelf coloring still worked); a non-privileged viewer sees non-private OR granted; `actor=None` stays conservative (non-private only) so a missing viewer never leaks a private name. Every graph builder threads its request `actor` through. |
